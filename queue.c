...
`capacity`. Il numero di elementi effettivamente presenti in coda potrà
essere minore di `capacity` (vedi oltre). Si inizi realizzando una
coda di capacità prefissata costante (ad esempio, 100 elementi).
...
dall'estremità opposta. Questo significa che tutti gli incrementi di
...
Viene fornito un programma [queue-main.c](queue-main.c) che legge ed
esegue una sequenza di comandi da un file il cui nome viene passato
sulla riga di comando. I comandi e il loro significato sono riportati
nella Tabella 1.
...
/* Ritorna true (nonzero) se e solo se la coda è piena. */
...
/* Ridimensiona il buffer associato alla coda q alla nuova dimensione
